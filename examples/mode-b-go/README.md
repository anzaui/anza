...
```bash
cd examples/mode-b-go
go run . --dist ../../web/dist --port 8781
```

Defaults: `--dist` → `../../web/dist`, `--port` → `8781`.

## Smoke check

```bash
curl -sL http://127.0.0.1:8781/docs/intro/start/ | grep -E '<title>|<h1>|shadowrootmode="open"|/app.js'
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:8781/app.js
```
...
| Static `dist/` + optional template | Gin/Echo/framework requirement |
